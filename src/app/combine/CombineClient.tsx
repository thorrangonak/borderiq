...
export default function CombineClient({ countries }: CombineClientProps) {
    ...
    <div>
      ...
      <div className="rounded-xl sm:rounded-2xl bg-white/5 border border-white/10 p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8">
        <h2 className="text-base sm:text-lg font-semibold text-white mb-4 sm:mb-6 flex items-center gap-2">
          <Combine className="w-5 h-5 text-purple-400 shrink-0" />
          Select Passports to Combine
        </h2>
        ...
        {canAddMore && (
          <button
            type="button"
            onClick={addSlot}
            className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-purple-500/10 text-purple-400 border border-purple-500/20 hover:bg-purple-500/20 transition-colors text-sm font-medium"
          >
      ...
      {data && !loading && (
        <div className="space-y-6 sm:space-y-8">
          ...
          <div className="rounded-2xl bg-gradient-to-br from-purple-500/10 via-blue-500/10 to-teal-500/10 border border-purple-500/20 p-5 sm:p-8">
            <div className="flex items-center gap-2 mb-6">
              <Zap className="w-6 h-6 text-purple-400" />
              <h2 className="text-lg sm:text-xl font-bold text-white">Combined Passport Power</h2>
            </div>

            {/* Combined passports row */}
            <div className="flex flex-wrap items-center gap-2 mb-6">
              {data.countries.map((name, i) => {
                const code = getCode(name);
                return (
                  <div key={name} className="flex items-center gap-2">
                    {i > 0 && <Plus className="w-4 h-4 text-gray-500" />}
                    <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2">
                      <CountryFlag code={code} size="sm" />
                      <span className="text-white text-sm font-medium">{name}</span>
                      <span className="text-gray-500 text-xs">#{data.rankings[name]?.rank}</span>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-6">
              ...
              {/* Best Individual */}
              <div className="bg-white/5 rounded-xl p-4 text-center border border-white/10">
                <Fingerprint className="w-5 h-5 text-orange-400 mx-auto mb-1" />
                <div className="text-2xl font-bold text-orange-400">{data.maxIndividualScore}</div>
                <div className="text-gray-400 text-xs mt-1">Best Individual</div>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
              <div className="relative flex-1 max-w-sm">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2.5 bg-white/5 border border-white/10 rounded-lg text-sm text-white placeholder-gray-500 outline-none focus:border-purple-500/50"
                />
              </div>
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="bg-white/5 border border-white/10 rounded-lg text-sm text-white py-2.5 px-3 outline-none focus:border-purple-500/50"
                >
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-xs font-medium transition-colors ${
                    tableSort === "best" ? "bg-purple-500/15 text-purple-400" : "text-gray-400 hover:bg-white/5"
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-xs font-medium transition-colors ${
                    tableSort === "name" ? "bg-purple-500/15 text-purple-400" : "text-gray-400 hover:bg-white/5"
                  }`}
                >
            ...
            {filteredDestinations.length > showCount && (
              <div className="flex justify-center gap-3 mt-6">
                ...
                <button
                  ...
                  className="px-5 py-2.5 rounded-xl bg-purple-500/10 text-purple-400 border border-purple-500/20 hover:bg-purple-500/20 transition-colors text-sm font-medium"
                >
          ...
          <div className="text-center py-4">
            <p className="text-gray-500 text-sm mb-3">Want a side-by-side comparison instead?</p>
            <Link
              href="/compare"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-teal-500/10 text-teal-400 border border-teal-500/20 hover:bg-teal-500/20 transition-colors text-sm font-medium"
            >
